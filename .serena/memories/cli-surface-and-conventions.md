...
Commands & flags
- `arledge <group> <action>` command groups implemented in `arledge/cli.py`.
- JSON input accepted via `--model` (inline JSON string) or `--model-file` (path to a JSON file).
- Per-command `--json-schema` flag and top-level `arledge schema <name>` print Pydantic JSON Schema to stdout.
...
Output conventions
- Machine-actionable outputs (created/listed models, schema) are printed to stdout as canonical JSON using `arledge/config.dump_model()`.
