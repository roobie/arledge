...
def test_init_db_adds_columns_when_missing(tmp_path):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "CREATE TABLE IF NOT EXISTS arledge_invoice (id INTEGER PRIMARY KEY AUTOINCREMENT, customer_id INTEGER NOT NULL, status TEXT NOT NULL, created_at TEXT NOT NULL)"
        )
        conn.commit()
        conn.close()
