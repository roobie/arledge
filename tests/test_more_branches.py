...
def test_db_prefixed_validation_and_bad_metadata(tmp_path):
    ...
    cur.execute(
        ...
        (
            "Z",
            config.dt_to_iso_utc(db.datetime.now(db.timezone.utc))
            if hasattr(db, "datetime")
            else config.dt_to_iso_utc(None),
        ),
    )
    cred_id = cur.lastrowid
    pa_table = db.prefixed("creditor_payment_account")
    cur.execute(
        f"INSERT INTO {pa_table} (creditor_id,type,metadata,created_at) VALUES (?,?,?,?)",
        (cred_id, "b", "not-a-json", config.dt_to_iso_utc(datetime.now(timezone.utc))),
    )
    conn.commit()
    conn.close()
