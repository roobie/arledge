...
def create_invoice(customer_id: int, lines: List[Dict[str, Any]], status: str = 'draft', due_days: Optional[int] = None, description: Optional[str] = None) -> int:
    ...
    if due_days is not None:
        due_at = (datetime.datetime.utcnow() + datetime.timedelta(days=int(due_days))).date().isoformat()
    conn = get_conn()
    cur = conn.cursor()
    cur.execute('INSERT INTO invoices (customer_id,status,created_at,due_at,description) VALUES (?,?,?,?,?)',
    ...
    for line in lines:
        ...
        cur.execute('INSERT INTO invoice_lines (invoice_id,description,quantity,unit_price,vat_rate) VALUES (?,?,?,?,?)',
...
def get_invoice(invoice_id: int) -> Optional[Dict[str, Any]]:
    ...
    if not row:
        ...
        return None
    inv = dict(row)
    cur.execute('SELECT description,quantity,unit_price,vat_rate FROM invoice_lines WHERE invoice_id=?', (invoice_id,))
    lines = cur.fetchall()
    inv_lines=[]
    subtotal=0.0
    total_vat=0.0
