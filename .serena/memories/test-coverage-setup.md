...
- Pytest default addopts updated to run coverage: `--cov=arledge --cov-report=xml:coverage.xml --cov-report=html:htmlcov --cov-fail-under=75` (stored in `tool.pytest.ini_options.addopts` in `pyproject.toml`).
- Coverage config: `.coveragerc` added to omit `tests/*`, `*/__init__.py`, `arledge/migrations/*`, `.venv/*`, and `*/__pycache__/*` and set HTML output directory.
