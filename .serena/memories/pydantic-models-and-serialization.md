...
Key points
- Models live in `arledge/models.py` and include `Customer`, `Creditor`, `PaymentAccount`, `InvoiceLine`, `Invoice`.
...
- Schema: `Model.model_json_schema()` is used by `--json-schema` and `arledge schema`.
- model_version: all models include a `model_version` field (default package `__version__`) to aid compatibility/migrations.
- Serialization: use `arledge/config.dump_model()` for CLI and export outputs to ensure:
