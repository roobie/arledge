...
def test_create_and_list_creditor_and_payment_account():
    ...
    pa = models.PaymentAccount(creditor_id=created.id, type='bank', label='L', identifier='I')
    ...
    pa3 = models.PaymentAccount(creditor_id=created.id, type='bank', is_default=True, identifier='D')
...
def test_customers_and_invoices_and_export(tmp_path):
    ...
    inv = models.Invoice(customer_id=created.id, lines=[models.InvoiceLine(description='one', unit_price=Decimal('10.00'))])
